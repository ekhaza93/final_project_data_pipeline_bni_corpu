...
=========================
ETL pipeline: fraud_labels.csv → stg_fraud_labels → fact_fraud_labels

Task flow:
    create_tables  (SQLExecuteQueryOperator) : DDL stg_fraud_labels & fact_fraud_labels
    extract_load   (@task Python)            : baca CSV → stg_fraud_labels
    transform      (SQLExecuteQueryOperator) : stg_fraud_labels → fact_fraud_labels
...
# ─── Konstanta ────────────────────────────────────────────────────────────────
CONN_ID     = "etlyu"  # <-- ganti dengan koneksi database yang sudah dibuat di airflow
...
@dag(
    ...
    description         = "ETL fraud_labels.csv → stg_fraud_labels → fact_fraud_labels",
    default_args        = {
        "owner"           : "airflow",
        "retries"         : 1,
    ...
    schedule            = None,
    catchup             = False,
    tags                = ["etl", "fraud_labels", "fact", "postgresql"],
...
def dag_etl_fraud_labels():
    ...
    # ── Task 3: Transform stg_fraud_labels → fact_fraud_labels ───────────────
